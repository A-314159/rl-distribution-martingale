...
import numpy as np
def num_tol(dtype: tf.DType):
    finfo = np.finfo(np.float32) if dtype == tf.float32 else np.finfo(np.float64)
    eps = tf.constant(finfo.eps, dtype)
    return {
        "denom_atol":  100.0 * eps,
        "sTy_fac":     100.0 * eps,
        "cos_tol":     100.0 * eps,
        "gamma_min":   tf.sqrt(eps),
        "powell_kappa":1000.0 * eps,
        "adam_eps":    tf.constant(1e-8 if dtype == tf.float32 else 1e-12, dtype),
        "cubic_det":   tf.pow(eps, 0.75),
    }
    ...
    def _build_inner_runner(self):
        ...
        def inner_k_steps(theta, f, g, gamma, k: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor, tf.Tensor, tf.Tensor,
            ...
            def body(it, theta, f, g, gamma, alpha_last, ls_last, mem_len, sTy_bar_last, cos_sy_last):
                ...
                d0_diag = None
                if cfg.use_adam_h0:
                    tolopt = num_tol(opt_dtype)
                    cfg_de = tf.cast(cfg.diag_eps, opt_dtype)
                    eps_adam = tf.cond(cfg_de > 0, lambda: cfg_de, lambda: tolopt["adam_eps"])
                    d0_diag = self._adam_diag(g, tf.cast(cfg.beta2, opt_dtype), eps_adam)
